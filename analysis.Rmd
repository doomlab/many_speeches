...
write.csv(intensity_final, "intensity_df.csv", row.names = F)
```
...
# probably have to pick the highest intensity for each combination? 

table(intensity_DF$typicality, useNA = "ifany")
...
intensity_model <- lme(Intensity ~ typicality, 
                       data = intensity_DF,
                       random = ~typicality | RepNo/filename)
...
## Hanging on to the code in case we need it 

```{r textgrids}

# put the textgrids in a textgrids folder 
inputFolder <- here::here("textgrids")

# find all the textgrid files within that folder 
listFiles <- list.files(path = inputFolder, 
                        pattern = "\\.TextGrid$" , ignore.case = TRUE)

# store textgrids as a list for each participant 
x <- list() 

# loop over the files and import the text grid 
